...
for tick in tickers:
    df = pd.DataFrame(get_daily('UAA','full'))
    ...
    sma = 50
    lma = 200
    df['sma']= pd.Series(df['close']).rolling(sma).mean()
    df['lma']= pd.Series(df['close']).rolling(lma).mean()
    ...
    df['Williams'] = ta.WILLR(df.high.values, df.low.values, df.close.values, timeperiod=12)
    ...
    df['1day_Signal'][df['1day_returns'].shift(-1) < 0] = -1
    df['5day_Signal'][df['5day_returns'].shift(-1) > 0] = 1
    df['5day_Signal'][df['5day_returns'].shift(-1) < 0] = -1
    df['10day_Signal'][df['10day_returns'].shift(-1) > 0] = 1
    df['10day_Signal'][df['10day_returns'].shift(-1) < 0] = -1
    df['20day_Signal'][df['20day_returns'].shift(-1) > 0] = 1
    df['20day_Signal'][df['20day_returns'].shift(-1) < 0] = -1
    df['60day_Signal'][df['60day_returns'].shift(-1) > 0] = 1
    df['60day_Signal'][df['60day_returns'].shift(-1) < 0] = -1


    #plot market returns - only for the plot not
    df['1day_mkt_returns'] = df['1day_returns'].expanding().sum()
    df['5day_mkt_returns'] = df['5day_returns'].expanding().sum()
    df['10day_mkt_returns'] = df['10day_returns'].expanding().sum()
    df['20day_mkt_returns'] = df['20day_returns'].expanding().sum()
    df['60day_mkt_returns'] = df['60day_returns'].expanding().sum()

    #Confirm by looking at " perfect strategy returns" i.e. buy if price will go up in future

    df['1day_str_returns'] = df['1day_Signal'].shift(1)* df['1day_returns']
    df['5day_str_returns'] = df['5day_Signal'].shift(1)* df['5day_returns']
    df['10day_str_returns'] = df['10day_Signal'].shift(1)* df['10day_returns']
    df['20day_str_returns'] = df['20day_Signal'].shift(1)* df['20day_returns']
    df['60day_str_returns'] = df['60day_Signal'].shift(1)* df['60day_returns']
    #Periods = ['1','5','10','20','60']
    '''for PER in Periods:
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(df[PER+'day_str_returns'].expanding().sum(), color='r', label=PER+" day Perfect Strategy Returns")
        ax.plot(df[PER+'day_returns'].expanding().sum(), color='b', label=PER+" day Market Returns")
        plt.legend(loc='best')
    ...
    for Sig in SIGNAL:
        ...
        data = data[[str(Sig)+'day_Signal','open', 'high', 'low', 'close', 'volume', 'sma', 'lma', 'macd1', 'macdSignal1', 'macdHist1', 'Williams', 'MFI', 'SAR', 'slowk', 'slowd', 'fastk', 'fastd', 'NAT', 'TRIX', 'PLUS_DI', 'MOM', 'MINUS_DI', 'BOP', 'ADX', 'TRANGE', 'OBV', 'ADOSC', '1day_returns', '5day_returns','10day_returns', '20day_returns', '60day_returns']]
        ...
        y_pred[y_pred == 'NA'] = 0
        strat_returns = pd.Series(y_pred).shift(1)* X[str(Sig)+'day_returns'][X.index >= '2017-01-01'].values
        #get the returns of the perfect strategy
        y_test[y_test == 'BUY'] = 1
        y_test[y_test == 'SELL'] = -1
        y_test[y_test == 'NA'] = 0
        perf_returns = pd.Series(pd.Series(y_test).shift(1)* X[str(Sig)+'day_returns'][X.index >= '2017-01-01'].values).reset_index(drop=True)
        ...
        plt.legend(loc='best')
        plt.title(str(tick)+" Algo Test Returns")
        plt.show()
